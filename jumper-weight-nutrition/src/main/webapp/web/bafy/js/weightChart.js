 ...
 */
//获取数据
var safeWeightList = [], minSafeList = [], maxSafeList = [], avgSafeList = [], userWeightList = [], hisWeightList = [];
...
function initWeightChart(data) {
	...
	//图表数据获取
	$.post(basePath + "/hisInfo/getWeightChartData", data, function(ret) {
		if (ret.msg != 1) {
		...
		hisWeightList = ret.data.hisWeightList;
    ...
    $('#container-weight').highcharts({
        ...
        tooltip: {
            ...
            formatter:function() {
                ...
                if (this.series != null && this.series.name == "孕妇记录") {//代表体重值的点
                	s += '<br><b>' + this.series.name + '：</b>' + this.y.toFixed(1) + 'kg';
                }
                if (this.series != null && this.series.name == "产检记录") {//代表产检记录体重值的点
                	s += '<br><b>' + this.series.name + '：</b>' + this.y.toFixed(1) + 'kg';
				}
                return s;
            }
