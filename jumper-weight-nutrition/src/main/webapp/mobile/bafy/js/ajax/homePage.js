...
$(function() {
	...
	//查询，如果用户没有在医院进行过就诊，强制提示且不显示用户首页信息
	//判断用户是初诊还是复诊状态
...
//	if(outpType != 0 && outpType != 1){
		//通过登记号查询用户信息，调his接口更新记录
		$.ajax({
...
//	}
	...
	$(document).on("click","#dietSurvey",function(){
		window.location.href = "dietSurvey.html?skipType=home&registerNum="+registerNum;
	});
	
	
	//点击运动记录
	$(document).on("click","#sportSurvey",function(){
		window.location.href = "sportSurvey.html?skipType=home&registerNum="+registerNum;
	});
