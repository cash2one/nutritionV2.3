...
public class UserWeightRecordServiceImpl extends BaseServiceImpl<UserWeightRecord> implements UserWeightRecordService {
	...
	public VoWeightChart getWeightChartData(int userId, int type, int hospitalId) {
		...
		if (type == 0) {//表示近7天
			...
			int[] max = FunctionUtils.calPregnantWeek(endDate, expDate);//第七天对应的孕周
			int[] min = FunctionUtils.calPregnantWeek(startDate, expDate);//第一天对应的孕周
	...
	@Override
	public boolean saveWeightByDate(Double weight, Date date, int userId, int hospitalId) {
		UserWeightRecord record = new UserWeightRecord();
		VoUserInfo voUser = userInfoService.findVoUserByUId(userId, hospitalId);
		
		record.setUserId(userId);
		record.setAverageValue(weight);
		int[] pweek = FunctionUtils.calPregnantWeek(date, TimeUtils.convertToDate(voUser.getExpectedDate()));
		double[] safeWeight = WeightFormula.getSafeWeight(voUser.getBmi(), voUser.getWeight(), pweek[2], 0, 0);
		int status = 1;//正常
		...
		record.setWeightState(status);
		record.setTestWeek(pweek[0]);
		record.setTestDay(pweek[1]);
		record.setTestTime(date);
		record.setAddTime(date);
		record.setCurrentIdentity(voUser.getCurrentIdentity());
		record.setBasalMetabolism(0);
		record.setBodyFatRate(0D);
		record.setMuscle(0D);
		record.setMoistureContent(0D);
		record.setHospitalId(0);
		
		String nowDate = TimeUtils.dateFormatToString(date, Const.YYYYMMDD);
		UserWeightRecord info = userWeightRecordMapper.findWeightRecordByDate(nowDate, userId);
		try {
			if (info != null) {//更新
				record.setId(info.getId());
			...
			// 监护表插入数据,配合app插入UserRecordsSets这张表一条数据
			UserRecordsSets sets = new UserRecordsSets();
			sets.setAddTime(date);
			sets.setData(weight.toString());
			sets.setIsHandle(false);
			sets.setObjectId(record.getId());
			sets.setRecordState(-1);
			sets.setRecordType(5);//5：代表体重
	...
	@Override
	public Double findLastWeight(int hospitalId, int userId) {
		//用户最近的一条体重记录
		UserWeightRecord weight = userWeightRecordMapper.findUserLastWeight(userId);
		if (weight != null) {
			return weight.getAverageValue();
		}
		HospitalUserInfo user = hospitalUserInfoMapper.findHospUserByUIdHospId(userId, hospitalId);
		if (user != null) {
