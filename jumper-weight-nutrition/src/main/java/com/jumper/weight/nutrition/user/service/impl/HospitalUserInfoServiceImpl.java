...
public class HospitalUserInfoServiceImpl extends BaseServiceImpl<HospitalUserInfo> implements HospitalUserInfoService {
	...
	public boolean addOrUpdateHospUser(HospitalUserInfo hospUser) {
		...
		try {
			if (info != null) {
				...
				//添加
				if (hospUser.getOutpatientNum() == null) {
					hospUser.setOutpatientNum("");//防止空指针
				}
	...
	@Override
	public HospitalUserInfo findHUserByRegNum(int hospitalId, String registerNum) {
		HospitalUserInfo hUser = hospitalUserInfoMapper.findByRegNum(hospitalId, registerNum);
		return hUser;
	}
}
