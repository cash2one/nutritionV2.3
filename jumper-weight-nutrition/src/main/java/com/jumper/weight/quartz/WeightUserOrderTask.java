...
public class WeightUserOrderTask {
	...
	public void synchorWeightUser() {
		try {
			...
			for (UserWeightRecord record : weightList) {
				...
				//设置未称重类型
				int weightType = 0;
				int day = TimeUtils.getTowDateMinusDay(new Date(), record.getAddTime());
				...
				if (updBean != null) {
					f1 = updBean.getCurrentWeight().equals(record.getAverageValue());
					f2 = updBean.getWeightExceptionType() == weightType;
					f3 = updBean.getLastWeightTime().compareTo(record.getAddTime()) == 0;
					if (f1 && f2 && f3) {
						continue;
					}
					bean.setId(updBean.getId());
				}
				bean.setCurrentWeight((f1) ? null : record.getAverageValue());
				bean.setWeightExceptionType((f2) ? null : weightType);//取枚举值未称重类型
				bean.setLastWeightTime((f3) ? null : record.getAddTime());
				if (updBean != null) {
					updList.add(bean);
				} else {
			...
			//所需更新医院用户表的集合
			List<HospitalUserInfo> updHUList = getUpdHUList(weightList);
			//添加或更新排序表
	 ...
	 * @return
	 */
	private List<HospitalUserInfo> getUpdHUList(List<UserWeightRecord> weightList) {
		//体重营养系统医院用户信息表
		...
		for (HospitalUserInfo hU : hUserList) {
			...
			//获取上次称体重时对应的孕周
			int[] weiPweek = FunctionUtils.calPregnantWeek(wei.getAddTime(), hU.getExpectedDate());
			//获取上次秤体重状态
			double[] safeWeight = WeightFormula.getSafeWeight(bmi, hU.getWeight(), weiPweek[2], type, hU.getPregnantType());
			int weightStatus = FunctionUtils.getStatusByWeight(safeWeight, wei.getAverageValue());
